...
function renderTable(table, laypage, formData) {
	var url = COMMON.filterURL('/v2/receivables/list');
    table.render(COMMON.renderSetup({
        ...
        where: formData,
        cols: [[ //表头
           {field: 'receiv_year', title: '回款年', width: 80}
          ,{field: 'receiv_month', title: '回款月', width: 80}
          ,{field: 'receiv_day', title: '回款日', width: 80}
          ,{field: 'receiv_amount', title: '回款金额', width: 110}
          ,{field: 'com_b_name', title: '单位名称', width: 180}
          ,{field: 'com_a', title: '所属公司', width: 80}
          ,{field: 'subject_type', title: '收入类型', width: 80}
          ,{field: 'invoice_num', title: '发票号', width: 80}
          ,{field: 'settlement_year', title: '结算年', width: 80}
          ,{field: 'settlement_month', title: '合计计算月', width: 100}
        ...
        parseData: function (res) {
            ...
            $('.layui-table input[data-type="update"]').on('click', function () {
                ...
                layer.open({
                    ...
                    area: ['1100px', '600px'],
                    shadeClose: true,
                    content: 'receivables_add.html?id=' + treId
                });
            });
            // 删除
            $('.layui-table input[data-type="delete"]').on('click', function () {
                ...
                var treId = $(this).data('id');
                $.ajax({
                    url: COMMON.filterURL('/v2/receivables/del'),
                    type: 'get',
                    data: {
                        id: treId
                    },
...
function windOpen(layer, params, opts) {
    ...
    layer.open({
        ...
        title: (opts.title || '回款信息录入'),
        content: (opts.content || [('receivables_add.html' + params)])
    });
}
